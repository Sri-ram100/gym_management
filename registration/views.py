...
def total_members_count(request):
    members_count = Registers.objects.count()
...
def gender_count(request):
    ...
    return HttpResponse(f"<script>alert('Total Male : {male_count}, Total Female: {female_count}, Total Others: {others_count}');window.location.replace('/home');</script>")
